define(function(require, exports, module) {
  ...
  var clientAli = UA.match(/\aliapp\(([^\/]+)\/([\d\.\w]+)\)/);
  var clientXiami = UA.match(/(aliapp)\((xm)\/([0-9.]+)\)\s*/);
  ...
  function share(obj) {
    return 'xiami://share?title=' + encodeURIComponent(obj.title) + '&url=' + encodeURIComponent(obj.url) + '&pic=' + encodeURIComponent(obj.pic) + '&content=' + encodeURIComponent(obj.content);
  }

  function login(url) {
    // if (client.weixin || client.weibo) {
    //   $('#J_clientTips').show();
    //   return;
    // }
    location.href = 'xiami://login?redirect=' + encodeURIComponent(url + '#login');
    //var surl = 'xiami://login?redirect=' + encodeURIComponent(url + '#login');
    //applink(APPSTORE, location.href, surl);
  }
  ...
  function applink(fail, curUrl, surl) {
    if (client.weixin || client.weibo) {
      $('#J_clientTips').show();
      return;
    }
    ...
    if (isAndroid) {
      ...
      if (~!doc.referrer.indexOf(curUrl) < 0) {
        location.href = surl;
